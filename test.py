...
class MarketAnalysis:
    ...
    def predict_price(self):
        ...
        short_term_momentum = (self.prices[-1] / self.prices[-3] - 1) * 100 if len(self.prices) > 3 else 0
        long_term_momentum = (self.prices[-1] / self.prices[-7] - 1) * 100 if len(self.prices) > 7 else 0
    ...
    try:
        ...
        if 'pairs' in response and response['pairs']:
            ...
            for i in range(14):
                ...
                timestamp = int((datetime.now() - timedelta(days=13-i)).timestamp() * 1000)
...
def get_agent_response(message):
    ...
    elif any(market_term in message for market_term in ['market', 'crypto', 'price', 'trend']):
